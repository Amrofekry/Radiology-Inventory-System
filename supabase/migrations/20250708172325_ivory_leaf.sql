-- Seed data for Radiology Equipment Inventory System
-- This script populates the database with initial reference data and sample equipment
...
SELECT setval('equipment_types_equipment_type_id_seq', (SELECT MAX(equipment_type_id) FROM equipment_types));
SELECT setval('funding_sources_funding_id_seq', (SELECT MAX(funding_id) FROM funding_sources));
